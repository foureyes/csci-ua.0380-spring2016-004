...
* __You will have to use the keyword, <code>nonlocal</code>, to implement this decorator.__ <code>nonlocal</code> is similar to <code>global</code>, but instead of affected global identifiers, it affects variables in a scope that encloses the current scope. [Please read this stack overflow article to on how to use <code>nonlocal</code>](http://stackoverflow.com/a/309000).
...
1. Download [hw05/quizzical.py](quizzical.py) - contains <code>MultipleChoiceQuestion</code> and <code>Quiz</code>
1. Download [hw05/python_quiz.py](python_quiz.py) - this uses the classes above to run an interactive quiz (should only be a few lines long!)
2. Download [hw05/questions.csv](questions.csv) - source of questions for quiz
...
1. Download [hw05/gravity.py](gravity.py)
